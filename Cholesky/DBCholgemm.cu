...
size_t estimateGemmul8WorkspaceBytes() {
  ...
  for (int j = 0; j < n; j += k) {
    ...
    for (int i = j; i < j_end - 1 && i < n; i += nb) {
      ...
      int kdim_local = i + ib - j;
      if (kdim_local > 0) update_ws(below, ib, kdim_local);
    }

    if (j + k >= n) break;
    int tail = n - j - k;
    int kblk = (k < tail) ? k : tail;
    int kdim2 = j + k;
    if (tail > 0 && kblk > 0 && kdim2 > 0) update_ws(tail, kblk, kdim2);
  }
  return ws_max;
}
...
int run() {
  ...
  auto run_once = [&](bool do_debug,
      ...
      for (int j = 0; j < n; j += k) {
          ...
          if (kdim_local > 0) {
            ...
            if (use_gemmul8) {
              gemmul8::gemm<T>(cublas_handle,
                               ...
                               (size_t)n,
                               &one,
                               A + (i + ib) + (size_t)(i + ib) * n,
                               (size_t)n,
                               gemmul8_moduli,
                               gemmul8_fastmode,
                               gemmul8_work);
            } else {
              CHECK_Cublas(Ops<T>::GemmNT(cublas_handle, below, ib, kdim_local, &neg_one,
                                          A + (i + ib) + (size_t)j * n, n,
                                          A + (i + ib) + (size_t)j * n, n,
                                          &one,
        ...
        if (tail > 0 && kblk > 0) {
          ...
          if (use_gemmul8) {
            ...
            CHECK_Cublas(Ops<T>::GemmNT(cublas_handle, tail, kblk, kdim2, &neg_one,
                                        A + (j + k), n,
                                        A + (j + k), n,
                                        &one,
    ...
    if (total_ms_out) *total_ms_out = potrf_ms + trsm_ms + gemm_ms;
  ...
  for (int i = 0; i < warmup_runs; ++i) {
    if (run_once(false, nullptr, nullptr, nullptr, nullptr) != 0) return 1;
  }
  ...
  for (int i = 0; i < bench_runs; ++i) {
    ...
    if (run_once(do_debug, &total_ms, &potrf_ms, &trsm_ms, &gemm_ms) != 0) return 1;
  ...
  std::printf("Left-looking Cholesky (gemm update) avg: %.3f ms: %.6f TFLOPs\n",
