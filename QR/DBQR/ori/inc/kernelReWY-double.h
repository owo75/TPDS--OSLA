...
// 并行将 Q -> (I - Q)
//-----------------------------------------------
__global__ void kernelIminusQ_inplace(double* d_Q,
                                      int ldq,
                                      int m, int n)
{
...
__global__ void kernelLUDecompSharedV1(double* d_Q,  int ldq,
                                     double* d_Y,  int ldy,
                                     double* d_U,
                                     int n)
{
...
__global__ void kernelLUDecompSharedV2(double* d_Q, int ldq,  
                                    double* d_Y, int ldy,  
                                    double* d_U,  
                                    int n)  
{  
...
//-----------------------------------------------
__global__ void kernelLUDecompGlobal(double* d_Q, int ldq,  
                                     double* d_Y, int ldy,  
                                     double* d_U,  
                                     int n)  
{  
    ...
    // 执行结果写回 d_Y 和 d_U  
    if (row < n && col < n) {  
        double val = d_Q[col * ldq + row];  // 按列主序获取元素  
...
//-----------------------------------------------
__global__ void kernelTrsmRightUpperWARP(const double* __restrict__ d_U,
                                         int ldu,
                                         double* d_Q, // Q底部
                                         int ldq,
                                         double* d_Y, // 目标: (m×n), 写[m-n..m-1]行。
                                         int ldy,
                                         int m, int n, int offsetRow)
{
    ...
    // 读 Q(actualRow, 0..n-1)
    double local[kerWYMAX_N];              ///////////////怎么确定用的是寄存器还是全局内存
    for (int c=0; c<n; c++){
...
//-----------------------------------------------
__global__ void kernelTrsmRightLowerT(const double* __restrict__ d_L,
                                                 int ldL,
                                                 double* d_W,
                                                 int ldW,
                                                 double* d_Q,
...
void ReconstructWYKernel(
    double* d_Q,  // in/out: m×n, 被覆盖 => (I-Q)
    int ldq,
    double* d_W,  // out: m×n
    int ldw,
    double* d_Y,  // out: m×n, 前n行为 L, 后(m-n)行为 L2
    int ldy,
    double* d_U,  // out: n×n => U
    int m, int n)
{
    if(n>kerWYMAX_N){
