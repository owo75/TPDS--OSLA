...
int count = 0;
//
__global__ void copySubmatrixToFull(double* A, int lda, double* A2, int lda2,
...
//---------------------------------------------
void TSQRSegment(double* d_A, double* d_W, double* d_Y, double* d_R,
                 double* work1, double* work2, int ldwork1, int lda, int cols,
    ...
    if (cols == b) {
        ...
        startTimer();
#endif
        tsqr<double>(cublas_handle, rows, cols, d_A, lda, d_R, cols, work1,
                     ldwork1);
...
void IterativeQR(
    double* d_A, int m, int n, int nb, int b, double* d_W, double* d_Y,
    int lda,  // dW，dY，dA的ld都是lda
    double* d_Q, double* d_R,
    double*
        work1,  // d_Q, d_R,
    ...
    for (j = 0; j * nb < n; j++) {
        ...
        while (i <= nt / 2) {
            ...
            if (!flag) {
                ...
                auto gemmul8_work3 = gemmul8::workSize(currn - j * nb, prei,
                                                       m - currm, NUM_MODULI);
                double* gemmul8_work3_ptr = (double*)malloc(gemmul8_work3);
        ...
        if (currn != n) {
            ...
            startTimer();
#endif
            CHECK_CUBLAS(cublasDgeam(cublas_handle,
...
int main(int argc, char* argv[]) {
    ...
    CHECK_CUDA(cudaMalloc(
        &work1, ldwork1 * 64 * sizeof(double)));  // 这两行是核函数中需要用到的
    CHECK_CUDA(cudaMalloc(&work2, ldwork2 * n * sizeof(double)));
    CHECK_CUDA(cudaMalloc(&work3, m * nb * sizeof(double)));

    CHECK_CUDA(cudaMemcpy(d_A_original, d_A, sizeof(double) * m * n,
                          cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(d_A_cusolver, d_A, sizeof(double) * m * n,
                          cudaMemcpyDeviceToDevice));
    ...
    CHECK_CUBLAS(cublasDgemm(handles1.blas_handle, CUBLAS_OP_N, CUBLAS_OP_T, m,
    ...
    // QR分解
    CHECK_CUSOLVER(cusolverDnDgeqrf_bufferSize(handles1.solver_handle, m, n,
                                               d_A_cusolver, lda, &lwork));
    double* d_work;  // 工作区
    CHECK_CUDA(cudaMalloc((void**)&d_work, sizeof(double) * lwork));
    startTimer();
    cusolverDnDgeqrf(handle, m, n, d_A_cusolver, lda, d_tau, d_work, lwork,
                     NULL);
    // CHECK_CUDA(cudaDeviceSynchronize());
    ...
    // 计算 Q 所需的工作区大小
    int lwork_orgqr = 0;
    CHECK_CUSOLVER(cusolverDnDorgqr_bufferSize(handles1.solver_handle, m, n, n,
                                               d_A_cusolver, lda, d_tau,
                                               &lwork_orgqr));
    // 分配工作区
    double* d_work_orgqr;
    CHECK_CUDA(cudaMalloc((void**)&d_work_orgqr, sizeof(double) * lwork_orgqr));
    CHECK_CUSOLVER(cusolverDnDorgqr(handles1.solver_handle, m, n, n,
