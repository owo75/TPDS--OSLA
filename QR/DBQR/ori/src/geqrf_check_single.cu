...
int main(int argc, char* argv[]) {
    ...
    CHECK_CUDA(cudaMemcpy(d_A_cusolver, d_A, sizeof(float) * m * n, cudaMemcpyDeviceToDevice));
    ...
    // QR分解
    CHECK_CUSOLVER(cusolverDnSgeqrf_bufferSize(handles1.solver_handle, m, n, d_A_cusolver, lda, &lwork));  
    float* d_work; // 工作区  
    CHECK_CUDA(cudaMalloc((void**)&d_work, sizeof(float) * lwork));  
    startTimer();  
    cusolverDnSgeqrf(handle, m, n, d_A_cusolver, lda, d_tau, d_work, lwork, NULL);  
    // CHECK_CUDA(cudaDeviceSynchronize());
    float cusolverQR_time = stopTimer();  
    // 计算Q
    // 计算 Q 所需的工作区大小  
    int lwork_orgqr = 0;  
    CHECK_CUSOLVER(cusolverDnSorgqr_bufferSize(handles1.solver_handle, m, n, n, d_A_cusolver, lda, d_tau, &lwork_orgqr));  
    // 分配工作区  
    float* d_work_orgqr;  
    CHECK_CUDA(cudaMalloc((void**)&d_work_orgqr, sizeof(float) * lwork_orgqr));  
    CHECK_CUSOLVER(cusolverDnSorgqr(handles1.solver_handle, m, n, n, d_A_cusolver, lda, d_tau, d_work_orgqr, lwork_orgqr, NULL));  
