...
int count = 0;
//
__global__ void copySubmatrixToFull(float* A, int lda, float* A2, int lda2,
                                    int m_A2, int n_A2, int posi_m,
                                    int posi_n) {
...
//---------------------------------------------
void TSQRSegment(float* d_A, float* d_W, float* d_Y, float* d_R,
                 float* work1, float* work2, int ldwork1, int lda, int cols,
                 int rows, int b, cublasHandle_t cublas_handle) {
    ...
    if (cols == b) {
        ...
        startTimer();
#endif
        tsqr<float>(cublas_handle, rows, cols, d_A, lda, d_R, cols, work1,
                     ldwork1);
        ...
        dim3 gridDim((cols + threadsPerDim - 1) / threadsPerDim,
                     (cols + threadsPerDim - 1) / threadsPerDim);
        ...
        // 1. 递归调用：处理前 half_i 列
        TSQRSegment(d_A, d_W, d_Y, d_R, work1, work2, ldwork1, lda, half_i,
                    rows, b, cublas_handle);
        ...
        CHECK_CUBLAS(cublasSgemm(cublas_handle,
                                 ...
                                 half_i));
#else
        auto tsqr_dgemm_size1 =
            gemmul8::workSize(half_i, half_i, rows, NUM_MODULI);
        float* tsqr_degmm_work1;
        cudaMalloc(&tsqr_degmm_work1, tsqr_dgemm_size1);
        gemmul8::gemm(cublas_handle,
                      ...
                      &beta,
                      work1,  // temp的起始地址
                      half_i, NUM_MODULI, false, tsqr_degmm_work1);
        cudaFree(tsqr_degmm_work1);
#endif
#ifdef NUM_MODULI
        // 在计算A - Y*temp
        CHECK_CUBLAS(cublasSgemm(cublas_handle,
                                 CUBLAS_OP_N,  // Y
                                 CUBLAS_OP_N,  // temp
                                 rows, half_i, half_i, &minusAlpha,
                                 d_Y,    // Y的起始地址
                                 lda,    // lda
                                 work1,  // temp的起始地址
                                 half_i, &alpha, d_A + half_i * lda,
                                 lda  // lda
                                 ));
        cudaDeviceSynchronize();
#else
        auto tsqr_dgemm_size2 =
            gemmul8::workSize(rows, half_i, half_i, NUM_MODULI);
        float* tsqr_degmm_work2;
        cudaMalloc(&tsqr_degmm_work2, tsqr_dgemm_size2);
        gemmul8::gemm(cublas_handle,
                      CUBLAS_OP_N,  // Y
                      CUBLAS_OP_N,  // temp
                      rows, half_i, half_i, &minusAlpha,
                      ...
                      lda,    // lda
                      work1,  // temp的起始地址
                      half_i, &alpha, d_A + half_i * lda, lda, NUM_MODULI,
                      false, tsqr_degmm_work2);
        cudaDeviceSynchronize();
        cudaFree(tsqr_degmm_work2);
#endif
#ifdef TIMEFUNC
        GemmTime1 += stopTimer();
#endif
        ...
        // 3. 递归调用：处理后 half_i 列
        TSQRSegment(d_A + half_i + half_i * lda, d_W + half_i + half_i * lda,
                    d_Y + half_i + half_i * lda, d_R, work1, work2, ldwork1,
                    lda, half_i, rows - half_i, b, cublas_handle);
        ...
        CHECK_CUBLAS(cublasDgemm(cublas_handle,
                                 ...
                                 half_i));
#else
        auto gemmul8_work_last1 =
            gemmul8::workSize(half_i, half_i, rows, NUM_MODULI);
        float* gemmul8_work_last1_ptr;
        CHECK_CUDA(cudaMalloc(&gemmul8_work_last1_ptr, gemmul8_work_last1));
        gemmul8::gemm(cublas_handle,
                      CUBLAS_OP_T,  // Y1'
                      CUBLAS_OP_N,  // W
                      half_i, half_i, rows, &alpha,
                      ...
                      &beta,
                      work1,  // temp的起始地址
                      half_i, NUM_MODULI, false, gemmul8_work_last1_ptr);
        cudaFree(gemmul8_work_last1_ptr);
#endif
...
void IterativeQR(
    float* d_A, int m, int n, int nb, int b, float* d_W, float* d_Y,
    int lda,  // dW，dY，dA的ld都是lda
    float* d_Q, float* d_R,
    float*
        work1,  // d_Q, d_R,
                // work不需要lda，可以任意修改矩阵形状，只要矩阵大小不超过m*n
    ...
    for (j = 0; j * nb < n; j++) {
        ...
        while (i <= nt / 2) {
            ...
            int cols = i;
            // int colsnum = i;  //
            // 这个参数是保存当前循环开始的i，方便利用currm定位WY的起始地址，而不用分类讨论
            // TSQR + ReconstructWY
            TSQRSegment(d_A + currm + currn * lda, d_W + currm + currn * lda,
                        d_Y + currm + currn * lda, d_R, work1, work2, ldwork1,
                        lda, cols, rows, b, cublas_handle);
...
#endif
                // MatrixName = "W";
                // writeMatrixToCsvV2(d_W_current, lda, m - currm, prei,
                // fileName, MatrixName); MatrixName = "A"; writeMatrixToCsvV2(
                // d_A_current + prei * lda, lda, m - currm, nb - i, fileName,
                // MatrixName); 更新完整的块 维度 W，Y：(m - currm) * prei A:(m
                // - currm) * nb - i 起始坐标W,Y：currm, currn   A: currm, currn
                // + prei
#ifndef NUM_MODULI
                CHECK_CUBLAS(
...
#else
                auto gemmul_size =
                    gemmul8::workSize(prei, nb - i, m - currm, NUM_MODULI);
                float* gemmul8_work;
                cudaMalloc((void**)&gemmul8_work, gemmul_size);
                gemmul8::gemm(cublas_handle,
                              CUBLAS_OP_T,  // W'
                              CUBLAS_OP_N,  // A
                              prei, nb - i, m - currm, &alpha,
                              ...
                              &beta,
                              work1,  // temp的起始地址
                              prei, NUM_MODULI, false, gemmul8_work);
                cudaFree(gemmul8_work);
#endif
#ifdef NUM_MODULI
                // MatrixName = "work1";
                // writeMatrixToCsvV2(work1, prei, prei, nb - i, fileName,
                // MatrixName); MatrixName = "workhalf";
                // writeMatrixToCsvV2(work1, prei, prei, nb - i, fileName,
                // MatrixName); MatrixName = "Yhalf";
                // writeMatrixToCsvV2(d_Y_current, lda, m - currm, prei,
                // fileName, MatrixName); 在计算A - Y*temp
                CHECK_CUBLAS(cublasSgemm(cublas_handle,
                                         CUBLAS_OP_N,  // Y
                                         CUBLAS_OP_N,  // temp
                                         m - currm, nb - i, prei, &minusAlpha,
                                         d_Y_current,  // Y的起始地址
                                         lda,          // lda
                                         work1,        // temp的起始地址
                                         prei, &alpha, d_A_current + prei * lda,
                                         lda  // lda
                                         ));
#else
                auto gemmul8_work2 =
                    gemmul8::workSize(m - currm, nb - i, prei, NUM_MODULI);
                float* gemmul8_work2_ptr;
                cudaMalloc(&gemmul8_work2_ptr, gemmul8_work2);
                gemmul8::gemm(cublas_handle,
                              CUBLAS_OP_N,  // Y
                              CUBLAS_OP_N,  // temp
                              m - currm, nb - i, prei, &minusAlpha,
                              ...
                              lda,          // lda
                              work1,        // temp的起始地址
                              prei, &alpha, d_A_current + prei * lda, lda,
                              NUM_MODULI, false, gemmul8_work2_ptr);
                cudaFree(gemmul8_work2_ptr);
#endif
                MatrixName = "更新的A";
                // writeMatrixToCsvV2(d_A_current + prei * lda, m, m - currm, nb
                // - i, fileName, MatrixName); cudaDeviceSynchronize();
            ...
            //                 MatrixName = "更新的A";
            //                 writeMatrixToCsvV2(d_A_current + prei * lda, m, m
            //                 - currm, nb - i, fileName, MatrixName);
            // #endif
            // 更新W矩阵
            if (!flag) {
                // W = [W1, W - W1 * Y1' * W]  Y1' *
                // W部分可以用下面非0，前面的W和W1需要用整列 维度 W1: m - currm,
                // currn - j * nb, W: m - currm, prei 起始坐标 W1: currm, j *
                // nb, W: currm, currn
#ifdef TIMEFUNC
                startTimer();
#endif
#ifndef NUM_MODULI
                CHECK_CUBLAS(
                    cublasDgemm(cublas_handle,  // 2
                                CUBLAS_OP_T,    // Y1'
                                CUBLAS_OP_N,    // W
                                currn - j * nb, prei, m - currm, &alpha,
                                ...
                                currn - j * nb));
#else
                auto gemmul8_work3 = gemmul8::workSize(currn - j * nb, prei,
                                                       m - currm, NUM_MODULI);
                float* gemmul8_work3_ptr = (float*)malloc(gemmul8_work3);
                cudaMalloc(&gemmul8_work3_ptr, gemmul8_work3);
                gemmul8::gemm(cublas_handle,  // 2
                              CUBLAS_OP_T,    // Y1'
                              CUBLAS_OP_N,    // W
                              currn - j * nb, prei, m - currm, &alpha,
                              d_Y + currm + j * nb * lda,  // Y1的起始地址
                              ...
                              &beta,
                              work1,  // temp的起始地址
                              currn - j * nb, NUM_MODULI, false,
                              gemmul8_work3_ptr);
                cudaFree(gemmul8_work3_ptr);
#endif
#ifdef NUM_MODULI
                ...
                CHECK_CUBLAS(cublasSgemm(
                    ...
                    CUBLAS_OP_N,    // W1
                    CUBLAS_OP_N,    // temp
                    m - j * nb, prei, currm - j * nb, &minusAlpha,
...
#else
                auto gemmul8_work4 = gemmul8::workSize(
                    m - j * nb, prei, currm - j * nb, NUM_MODULI);
                float* gemmul8_work4_ptr;
                cudaMalloc(&gemmul8_work4_ptr, gemmul8_work4);
                gemmul8::gemm(cublas_handle,  //
                              CUBLAS_OP_N,    // W1
                              CUBLAS_OP_N,    // temp
                              m - j * nb, prei, currm - j * nb, &minusAlpha,
                              ...
                              currn - j * nb, &alpha,
                              d_W + j * nb + currn * lda,  // W的起始地址
                              lda, NUM_MODULI, false, gemmul8_work4_ptr);
                cudaFree(gemmul8_work4_ptr);
#endif
                cudaDeviceSynchronize();
#ifdef TIMEFUNC
                WYTime1 += stopTimer();
        ...
        if (currn != n) {
            ...
            writeMatrixToCsvV2(d_A_current, m, m - j * nb, n - (j + 1) * nb,
                               fileName, MatrixName);
            MatrixName = "更新前的W矩阵";
...
#else
            auto tsqr_dgemm_size1 =
                gemmul8::workSize(nb, n - (j + 1) * nb, m - j * nb, NUM_MODULI);
            float* tsqr_degmm_work1;
            cudaMalloc(&tsqr_degmm_work1, tsqr_dgemm_size1);
            gemmul8::gemm(cublas_handle,
                          CUBLAS_OP_N,  // W'
                          ...
                          &beta,
                          work2,  // temp的起始地址
                          nb, NUM_MODULI, false, tsqr_degmm_work1);
            cudaFree(tsqr_degmm_work1);
#endif
#ifdef TIMEFUNC
            float temptime1 = stopTimer();
            long long t =
                (long long)(m - j * nb) * (n - (j + 1) * nb) * (2LL * nb);
...
#else
            auto tsqr_dgemm_size3 =
                gemmul8::workSize(m - j * nb, n - (j + 1) * nb, nb, NUM_MODULI);
            float* tsqr_degmm_work3;
            cudaMalloc(&tsqr_degmm_work3, tsqr_dgemm_size3);
            gemmul8::gemm(cublas_handle,
                          CUBLAS_OP_N,  // Y
                          CUBLAS_OP_N,  // temp
                          m - j * nb, n - (j + 1) * nb, nb, &minusAlpha,
                          d_Y_current,  // Y的起始地址
                          lda,          // lda
                          work2,        // temp的起始地址
                          nb, &alpha,
                          d_A_current,  // A_sub的起始地址
                          lda, NUM_MODULI, false, tsqr_degmm_work3);
            cudaFree(tsqr_degmm_work3);
#endif
            cudaDeviceSynchronize();
...
int main(int argc, char* argv[]) {
    ...
    CHECK_CUDA(cudaMalloc((void**)&d_A_cusolver, sizeof(float) * m * n));
    ...
    CHECK_CUDA(cudaMalloc(
        &work1, ldwork1 * 64 * sizeof(float)));  // 这两行是核函数中需要用到的
    CHECK_CUDA(cudaMalloc(&work2, ldwork2 * n * sizeof(float)));
    CHECK_CUDA(cudaMalloc(&work3, m * nb * sizeof(float)));

    CHECK_CUDA(cudaMemcpy(d_A_original, d_A, sizeof(float) * m * n,
                          cudaMemcpyDeviceToDevice));
    CHECK_CUDA(cudaMemcpy(d_A_cusolver, d_A, sizeof(float) * m * n,
                          cudaMemcpyDeviceToDevice));
...
#endif
    IterativeQR(
        d_A, m, n, nb, b, d_W, d_Y, lda, d_Q, d_R, work1, work2, work3, ldwork1,
        ldwork2,
        cublas_handle);  // 这里的dR和dQ可以看作是work矩阵，并不是最后实际的R和Q
    cudaDeviceSynchronize();
#ifndef TIMEFUNC
    ...
    float totaltime = tsqrTime1 + reconstructWYTime1 + getUTime + GemmTime1 +
                      WYTime1 + TailMatrixTime1 + TailMatrixTime2 + transWTime;
    ...
    CHECK_CUBLAS(cublasSgemm(handles1.blas_handle, CUBLAS_OP_N, CUBLAS_OP_T, m,
                             m, n, &alpha, d_W, m, d_Y, m, &beta, d_Q, m));

    // 提取 R 矩阵
    dim3 gridDimMN((n + threadsPerDim - 1) / threadsPerDim,
    ...
    // cusolver计算qr分解计时
    int lwork = 0;  // 工作区大小
    float* d_tau;  // 存储 Householder 反射向量的数组
    ...
    // QR分解
    CHECK_CUSOLVER(cusolverDnSgeqrf_bufferSize(handles1.solver_handle, m, n,
                                               d_A_cusolver, lda, &lwork));
    float* d_work;  // 工作区
    CHECK_CUDA(cudaMalloc((void**)&d_work, sizeof(float) * lwork));
    startTimer();
    cusolverDnSgeqrf(handle, m, n, d_A_cusolver, lda, d_tau, d_work, lwork,
                     NULL);
    // CHECK_CUDA(cudaDeviceSynchronize());
    ...
    // 计算 Q 所需的工作区大小
    int lwork_orgqr = 0;
    CHECK_CUSOLVER(cusolverDnSorgqr_bufferSize(handles1.solver_handle, m, n, n,
                                               d_A_cusolver, lda, d_tau,
                                               &lwork_orgqr));
    // 分配工作区
    float* d_work_orgqr;
    CHECK_CUDA(cudaMalloc((void**)&d_work_orgqr, sizeof(float) * lwork_orgqr));
    CHECK_CUSOLVER(cusolverDnSorgqr(handles1.solver_handle, m, n, n,
                                    d_A_cusolver, lda, d_tau, d_work_orgqr,
                                    lwork_orgqr, NULL));
