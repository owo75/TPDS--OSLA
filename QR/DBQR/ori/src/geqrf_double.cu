...
int main(int argc, char* argv[]) {
    ...
    double* d_A = nullptr;
    double* d_A_original = nullptr;
    double* d_A_cusolver = nullptr;
    double* d_W = nullptr;
    double* d_Y = nullptr;
    double* d_Q = nullptr;
    double* d_R = nullptr;
...
#endif
    // d_W / d_Y are indexed with global column offsets up to n (currn/j*nb),
    // so they must be allocated as m*n, not m*nb.
    CHECK_CUDA(cudaMalloc(&d_W, sizeof(double) * static_cast<size_t>(m) * n));
    CHECK_CUDA(cudaMalloc(&d_Y, sizeof(double) * static_cast<size_t>(m) * n));
    // d_Q is unused in this benchmark path; avoid large unnecessary allocation.
    // CHECK_CUDA(cudaMalloc(&d_Q, sizeof(double) * m * m));
    CHECK_CUDA(cudaMalloc(&d_R, sizeof(double) * b * b));
    // work1 only needs ldwork1 * nb for current GEMM shapes.
    CHECK_CUDA(cudaMalloc(&work1, static_cast<size_t>(ldwork1) * nb * sizeof(double)));
    CHECK_CUDA(cudaMalloc(&work2, m * n * sizeof(double)));
    CHECK_CUDA(cudaMalloc(&work3, m * nb * sizeof(double)));
    
    CHECK_CUDA(cudaMemset(d_W, 0, static_cast<size_t>(m) * n * sizeof(double)));
    CHECK_CUDA(cudaMemset(d_Y, 0, static_cast<size_t>(m) * n * sizeof(double)));
    ...
    printf("tflops = %.4f\n", 4.0 * n * n *(m - 1.0/3 * n) / totaltime / 1e9);
#endif
    ...
    if (d_A_original) CHECK_CUDA(cudaFree(d_A_original));
