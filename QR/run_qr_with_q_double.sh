...
log="qr_with_q_double_${ts}.log"
echo "Logging to: ${log}"
exec > >(tee -a "$log") 2>&1

sizes=(8192 12288 16384 20480 24576 28672 32768)
warmup=2
repeat=5

csv="qr_with_q_double_${ts}.csv"
echo "CSV: ${csv}"
echo "n,geqrf_ms,geqrf_tflops,orgqr_ms,orgqr_tflops,total_ms,total_tflops" > "$csv"

run_one() {
  local n="$1"
  echo "==== double_${n} @ $(date +%F_%T) ===="
  local out
  out="$($bin "$n" double --with-q --warmup "$warmup" --repeat "$repeat")"
  echo "$out"
